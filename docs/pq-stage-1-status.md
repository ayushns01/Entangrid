...
This document records the current state of Entangrid's PQ-enabled line on `main` as of April 11, 2026.
...
In other words, the remaining work on this line is not "add PQ signing" or "add PQ transport." Those pieces are already integrated.

## Current Verification Snapshot

The line has been verified with the Stage 1 crate and smoke coverage:
...
The latest rigorous live matrix coverage on the current line is split across:
...
- because of that, this line should not yet be presented as fully signed off for final merge
...
- the current line still needs the last two `6`-validator bursty consensus proofs before final signoff
- once that closes, `main` can be treated as the signed-off Stage 1 PQ milestone
