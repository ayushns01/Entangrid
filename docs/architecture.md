...
- PQ Stage 1 now also includes hybrid validator identity enforcement, hybrid session establishment, encrypted post-handshake frame bodies, and transport-local session TTL turnover on `main`
...
Current PQ Stage 1 detail on `main`:
...
- an opt-in `require_hybrid_validator_signatures` flag now adds network-wide startup enforcement for hybrid validator identities and runtime enforcement for transaction, block, proposal-vote, relay-receipt, and service-attestation signatures
- service aggregates remain unsigned containers, but now inherit strict hybrid enforcement transitively because aggregate validation re-validates every embedded service attestation
