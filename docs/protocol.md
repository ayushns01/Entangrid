...
- on `main`, hybrid-signature enforcement is now available behind `require_hybrid_validator_signatures` for validator startup, transactions, blocks, proposal votes, relay receipts, and service attestations
...
Current PQ Stage 1E note on `main`:
...
Current PQ Stage 1I note on `main`:
...
Current PQ Stage 1J note on `main`:
