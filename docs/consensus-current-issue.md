# Current Consensus Issue On The PQ-Enabled `main` Line

Status date: 2026-04-11

This note explains the specific consensus problem currently blocking final signoff on the PQ-enabled `main` line.
...
Those areas are in much better shape on the current line.
...
- the current PQ-enabled `main` line no longer holds that earlier proof
- this should be treated as a current regression or, at minimum, an invalidated proof on the present line state
...
If these two scenarios remain red, the current line cannot honestly be described as:

- fully consensus-stable
- fully proven under the current localnet acceptance matrix
- ready for final Stage 1 signoff on `main`
...
The current line has real and substantial progress:
...
But the line still has two serious unresolved consensus problems:
