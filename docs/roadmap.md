...
- finish the remaining `baseline-6-bursty` and `gated-6-bursty` proofs on the current PQ-enabled line on `main`
- freeze the rigorous matrix as the acceptance gate once those last two scenarios pass
- treat the current `main` line as the signed-off Stage 1 PQ baseline after that
...
- Stage 1 PQ signing, transport, and hybrid localnet bootstrap are implemented on `main`
