...
use entangrid_types::{GenesisConfig, HashBytes, ValidatorConfig, ValidatorId, hash_many};
...
pub trait Signer: Send + Sync {
    fn sign(&self, validator_id: ValidatorId, message: &[u8]) -> Result<Vec<u8>>;
}

pub trait Verifier: Send + Sync {
    fn verify(&self, validator_id: ValidatorId, message: &[u8], signature: &[u8]) -> Result<bool>;
}
